  ...
  return (
    <div style={{ background: "#f8fafc", minHeight: "100vh", fontFamily: "'DM Sans', -apple-system, sans-serif" }}>
      <style>{`
        ...
        .dash-stat-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-bottom: 20px; }
        .dash-main-grid { display: grid; grid-template-columns: 1fr; gap: 16px; margin-bottom: 16px; }
        .dash-bottom-grid { display: grid; grid-template-columns: 1fr; gap: 16px; }
        .dash-orders-grid { display: grid; grid-template-columns: 1fr; gap: 0; }
        .dash-order-row-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; padding: 12px 16px; border-bottom: 1px solid #f8fafc; cursor: pointer; }
        .dash-header-row { display: none; }
        @media(min-width: 640px) {
          .dash-stat-grid { grid-template-columns: repeat(2, 1fr); gap: 16px; }
        }
        @media(min-width: 1024px) {
          ...
          .dash-orders-grid { grid-template-columns: 1.5fr 1.5fr 2fr 1fr 1fr 1fr; }
          .dash-order-row-grid { grid-template-columns: 1.5fr 1.5fr 2fr 1fr 1fr 1fr; padding: 16px 24px; }
          .dash-header-row { display: grid; grid-template-columns: 1.5fr 1.5fr 2fr 1fr 1fr 1fr; padding: 12px 24px; background: #fafafa; }
          .dash-mobile-label { display: none !important; }
        }
      `}</style>

      <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "16px sm:24px lg:32px" }} className="dash-pad">
        <style>{`.dash-pad { padding: 16px; } @media(min-width:640px){.dash-pad{padding:24px;}} @media(min-width:1024px){.dash-pad{padding:32px;}}`}</style>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px", animation: "slideUp 0.4s ease both", flexWrap: "wrap", gap: "12px" }}>
          <div>
            ...
            <h1 style={{ fontSize: "clamp(22px, 4vw, 32px)", fontWeight: 700, color: "#0f172a", fontFamily: "'DM Serif Display', serif", letterSpacing: "-0.02em", lineHeight: 1.1 }}>
        ...
        <div className="dash-stat-grid">
          {[
            ...
            { label: "Total Customers", value: stats.totalCustomers.toLocaleString(), accent: "#10b981", icon: <Users style={{ width: 18, height: 18 }} />, sub: `+${stats.newCustomersThisMonth} this month`, delay: "180ms" },
          ].map((s, i) => (
            <div key={i} style={{ background: "#fff", borderRadius: "16px", padding: "16px sm:20px lg:24px", border: "1px solid #f1f5f9", boxShadow: "0 1px 3px rgba(0,0,0,0.04)", position: "relative", overflow: "hidden", animation: `slideUp 0.5s ease both`, animationDelay: s.delay }}>
              <div style={{ position: "absolute", top: "-20px", right: "-20px", width: "70px", height: "70px", borderRadius: "50%", background: s.accent, opacity: 0.1 }} />
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
                <div style={{ width: "36px", height: "36px", borderRadius: "10px", background: s.accent + "15", display: "flex", alignItems: "center", justifyContent: "center", color: s.accent }}>{s.icon}</div>
                {s.change !== undefined && (
                  <div style={{ display: "flex", alignItems: "center", gap: "4px", padding: "3px 8px", borderRadius: "999px", background: s.change >= 0 ? "#dcfce7" : "#fee2e2", color: s.change >= 0 ? "#16a34a" : "#dc2626", fontSize: "11px", fontWeight: 600 }}>
                    {s.change >= 0 ? <ArrowUpRight style={{ width: 11, height: 11 }} /> : <ArrowDownRight style={{ width: 11, height: 11 }} />}{Math.abs(s.change)}%
                  </div>
                )}
              </div>
              <p style={{ fontSize: "12px", color: "#94a3b8", marginBottom: "4px", fontWeight: 500 }}>{s.label}</p>
              <p style={{ fontSize: "clamp(18px, 3vw, 26px)", fontWeight: 700, color: "#0f172a", letterSpacing: "-0.02em" }}>{s.value}</p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <div style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)", borderRadius: "20px", padding: "20px", animation: "slideUp 0.5s ease both", animationDelay: "240ms" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
                ...
                <p style={{ fontSize: "11px", color: "#64748b", fontWeight: 500, letterSpacing: "0.05em", textTransform: "uppercase" }}>Top product (30 days)</p>
              </div>
              {topProduct ? (
                <>
        ...
        <div style={{ background: "#fff", borderRadius: "20px", border: "1px solid #f1f5f9", boxShadow: "0 1px 3px rgba(0,0,0,0.04)", animation: "slideUp 0.5s ease both", animationDelay: "300ms", overflow: "hidden", marginBottom: "16px" }}>
          ...
          {/* Desktop header */}
          <div className="dash-header-row" style={{ borderBottom: "1px solid #f1f5f9" }}>
          ...
          {recentOrders.length === 0 ? (
            ...
            recentOrders.map((order, i) => {
              ...
              return (
                <div key={order.id} className="order-row dash-order-row-grid" onClick={() => navigate(`/orders/${order.id}`)} style={{ borderBottom: i < recentOrders.length - 1 ? "1px solid #f8fafc" : "none", cursor: "pointer", transition: "background 0.1s", alignItems: "center", animation: "slideUp 0.4s ease both", animationDelay: `${340 + i * 40}ms` }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    <p style={{ fontSize: "12px", fontWeight: 500, color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{order.customerName}</p>
                  </div>
                  <p style={{ fontSize: "12px", color: "#64748b", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} className="hidden lg:block">{productName}</p>
                  <p style={{ fontSize: "12px", fontWeight: 600, color: "#0f172a" }}>₹{order.totalAmount.toLocaleString()}</p>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: "4px", padding: "3px 8px", borderRadius: "999px", background: order.paymentStatus === "PAID" ? "#dcfce7" : "#fef3c7", color: order.paymentStatus === "PAID" ? "#166534" : "#92400e", fontSize: "10px", fontWeight: 600 }} className="hidden lg:inline-flex">
                ...
                <>
                  ...
                  {items.map((item) => (
                    <div key={item.label} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "9px 0", borderBottom: "1px solid #f8fafc" }}>
                      ...
                      <p style={{ fontSize: "13px", color: item.done ? "#94a3b8" : "#0f172a", fontWeight: item.done ? 400 : 500, textDecoration: item.done ? "line-through" : "none", flex: 1 }}>{item.label}</p>
