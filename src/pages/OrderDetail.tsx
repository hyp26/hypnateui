...
export const OrderDetail = () => {
  ...
  const { getOrder, updateOrderStatus, updatePaymentStatus, addTracking } = useOrderStore();
  ...
  const handleTrackingSubmit = () => {
    if (trackingInput.trim()) {
      addTracking(order.id, trackingInput);
  ...
  const currentStepIndex = steps.findIndex(s => s.id === order.status) === -1 
    ? (order.status === 'paid' ? 0 : order.status === 'payment_pending' ? 0 : 4) // Handle edge cases
    : steps.findIndex(s => s.id === order.status);

  // Find if order is cancelled
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            ...
            {showTrackingInput && (
              <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200 animate-in fade-in slide-in-from-top-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">Enter Tracking Details</label>
            ...
            <div className="relative space-y-6 pl-4 border-l-2 border-gray-100">
              {order.timeline.map((event, idx) => (
                <div key={idx} className="relative">
                  ...
                  <p className="text-xs text-gray-500">{format(new Date(event.timestamp), 'MMM dd, hh:mm a')}</p>
                </div>
              ))}
            </div>
          </div>
