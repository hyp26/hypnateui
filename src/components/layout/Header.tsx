...
import {
  ...
  CheckCheck, Trash2, ExternalLink, Menu,
} from 'lucide-react';
...
export const Header = ({ onMenuClick }: HeaderProps) => {
  ...
  const markRead = async (id: number) => {
    ...
    try { await api.patch(`/api/notifications/${id}/read`); } catch { await fetchNotifs(true); }
  };

  const markAllRead = async () => {
    setNotifs(prev => prev.map(n => ({ ...n, isRead: true })));
    setUnread(0);
    try { await api.patch('/api/notifications/read-all'); } catch { await fetchNotifs(true); }
  };

  const deleteNotif = async (id: number, e: React.MouseEvent) => {
    e.stopPropagation();
    setNotifs(prev => prev.filter(n => n.id !== id));
    try { await api.delete(`/api/notifications/${id}`); } catch { await fetchNotifs(true); }
  };

  const handleSearch = (val: string) => {
    setQuery(val);
    if (debounceRef.current) clearTimeout(debounceRef.current);
    if (!val.trim() || val.trim().length < 2) { setResults(null); setSearchOpen(false); return; }
  ...
  const hasResults = results && (results.orders.length + results.customers.length + results.products.length > 0);
  ...
  const navigateTo = (path: string) => { setSearchOpen(false); setQuery(''); setMobileSearchOpen(false); navigate(path); };

  return (
    <>
      <style>{headerCss}</style>

      <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-3 sm:px-6 fixed top-0 right-0 left-0 lg:left-64 z-20 shadow-sm gap-3">
        ...
        <div className={cn("flex-1 items-center gap-4", mobileSearchOpen ? "flex" : "hidden sm:flex")} ref={searchRef}>
          <div className="relative w-full sm:w-80 lg:w-96">
            ...
            {query && !searching && (
              <button
                ...
                onClick={() => { setQuery(''); setResults(null); setSearchOpen(false); setMobileSearchOpen(false); }}
              >
                <X size={13} />
              </button>
            )}
        ...
        <div className="flex items-center gap-1 sm:gap-3 shrink-0">
          ...
          <button
            onClick={toggleLang}
            className="hidden sm:flex p-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors items-center gap-1.5 text-sm font-medium"
          >
                  ...
                  {notifs.length === 0 ? (
                    ...
                    notifs.map(n => {
                      ...
                      return (
                        ...
                        >
                          <div className="notif-icon-wrap" style={{ background: bg, color }}>{notifIcon(n.type)}</div>
                          ...
                          <div className="notif-actions">
                            ...
                            <button className="notif-delete" onClick={e => deleteNotif(n.id, e)} title="Dismiss">
                              <Trash2 size={12} />
                            </button>
                          </div>
                        </div>
          ...
          <HeadlessMenu as="div" className="relative">
            <HeadlessMenu.Button className="flex items-center gap-2 outline-none p-1 sm:pr-3 rounded-full hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-200">
              <div className="h-8 w-8 rounded-full bg-primary-100 flex items-center justify-center text-primary-700 font-bold text-sm border border-primary-200 shrink-0">
...
.search-dropdown {
  ...
  box-shadow: 0 16px 48px rgba(0,0,0,0.12); z-index: 100; overflow: hidden;
...
.search-section-label { display: flex; align-items: center; gap: 5px; padding: 8px 14px 4px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.6px; color: #94a3b8; }
.search-item { display: block; width: 100%; text-align: left; padding: 9px 14px; border: none; background: none; cursor: pointer; transition: background 0.1s; }
...
.search-item-badge { font-size: 10px; font-weight: 600; padding: 1px 7px; border-radius: 100px; background: #f1f5f9; color: #64748b; }
.badge-warn { background: #fff7ed !important; color: #c2410c !important; }
.search-status { font-size: 10px; font-weight: 700; padding: 1px 8px; border-radius: 100px; text-transform: capitalize; }
.status-pending { background: #fff7ed; color: #c2410c; }
.status-confirmed, .status-delivered { background: #f0fdf4; color: #166534; }
.status-cancelled { background: #fef2f2; color: #991b1b; }
.search-footer { padding: 10px 14px; border-top: 1px solid #f1f5f9; display: flex; justify-content: flex-end; }
.search-footer-link { display: inline-flex; align-items: center; gap: 4px; font-size: 12px; font-weight: 600; color: #0d9488; background: none; border: none; cursor: pointer; }

.notif-badge { position: absolute; top: 4px; right: 4px; min-width: 16px; height: 16px; background: #f97316; color: #fff; font-size: 9px; font-weight: 800; border-radius: 100px; border: 2px solid #fff; display: flex; align-items: center; justify-content: center; padding: 0 3px; line-height: 1; }

.notif-panel { position: absolute; right: 0; top: calc(100% + 10px); width: min(360px, calc(100vw - 1rem)); background: #fff; border: 1.5px solid #e2e8f0; border-radius: 16px; box-shadow: 0 16px 48px rgba(0,0,0,0.12); z-index: 100; overflow: hidden; animation: dropIn 0.15s ease; }
...
.notif-mark-all { display: flex; align-items: center; gap: 4px; font-size: 11px; font-weight: 600; color: #0d9488; background: none; border: none; cursor: pointer; padding: 4px 8px; border-radius: 6px; transition: background 0.15s; }
...
.notif-empty { display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 40px 20px; color: #cbd5e1; font-size: 13px; }
.notif-item { display: flex; gap: 10px; align-items: flex-start; padding: 12px 16px; cursor: pointer; border-bottom: 1px solid #f8fafc; transition: background 0.1s; position: relative; }
.notif-item:hover { background: #f8fafc; }
.notif-item:hover .notif-delete { opacity: 1; }
.notif-item-unread { background: #f0fdfa; }
.notif-icon-wrap { width: 32px; height: 32px; border-radius: 8px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
...
.notif-delete { background: none; border: none; cursor: pointer; color: #cbd5e1; padding: 2px; border-radius: 4px; opacity: 0; transition: opacity 0.15s, color 0.15s; }
.notif-delete:hover { color: #ef4444; }
.notif-footer { padding: 10px 16px; border-top: 1px solid #f1f5f9; display: flex; justify-content: center; }
.notif-footer-link { display: inline-flex; align-items: center; gap: 4px; font-size: 12px; color: #94a3b8; text-decoration: none; transition: color 0.15s; }
